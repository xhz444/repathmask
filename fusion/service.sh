...
json_escape() {
	# JSON strings must not contain raw control characters. The module
	# prop can come from a CRLF zip, so escape CR/LF as well as slash/quote.
...
package_to_uid_from_packages_list() {
	...
	while IFS= read -r _line || [ -n "$_line" ]; do
		set -- $_line
		[ "$1" = "$_pkg" ] || continue
		case "$2" in ''|*[!0-9]*) return 0 ;; *) printf '%s\n' "$2"; return 0 ;; esac
...
while IFS= read -r pkg; do
	...
	# Resolve the app UID from the package database. Shared-storage
	# backing dirs are often media_rw-owned and are not a valid source.
